...
class resnet(_fasterRCNN):
  def __init__(self, classes, num_layers=101, pretrained=True, class_agnostic=False):
    ...
    _fasterRCNN.__init__(self, classes, class_agnostic, multi_feat=False)
  ...
  def _init_modules(self):
    if self.num_layers==101:
      ...
      raise NotImplementedError('no resnet backbone...')
...
def optimistic_restore(network, state_dict, whichs=None):
    ...
    for name, param in state_dict.items():
        if whichs is not None and whichs not in name:
          continue
